...
int imgproc_tile(struct Image *input_img, int n, struct Image *output_img) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            // Calculate the corresponding source pixel from the input image
            // We divide by n to get the correct tile, then multiply by n to sample every nth pixel
            int src_x = (x % tile_width) + (x / tile_width) * tile_width;
            int src_y = (y % tile_height) + (y / tile_height) * tile_height;
