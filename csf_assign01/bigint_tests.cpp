...
void test_assignment_operator(TestObjs *objs) {
    ...
    ASSERT(b.to_hex() == "10000000000000000");  
    ...
    BigInt e = objs->u64_max;  
    ASSERT(e.to_hex() == "ffffffffffffffff");  

    // Test case 7: Assigning a BigInt to another after modification
    a = objs->negative_two_pow_64; 
    ASSERT(a.to_hex() == "-10000000000000000");  
...
void test_get_bit_vector(TestObjs *objs) {
    ...
    // Test case 5: Check the bit vector for a negative number 
    ...
    // Test case 6: Check the bit vector for a larger negative number
    ...
    // Test case 7: Check if modifying the returned reference modifies the original BigInt
    // verify that the reference indeed reflects the underlying bits without modification.
...
void test_default_constructor(TestObjs *objs) {
    ...
    // Check that the magnitude contains only one word and that word is 0
    const std::vector<uint64_t>& bits = default_bigint.get_bit_vector();
    ASSERT(bits.size() == 1);  // bit vector should contain exactly one word
    ASSERT(bits[0] == 0);      

    // Check that the BigInt is not marked as negative
    ASSERT(!default_bigint.is_negative());  // BigInt should not be negative
...
void test_initializer_list_constructor(TestObjs *objs) {
    ...
    ASSERT(bits3[0] == 0x123456789ABCDEF0UL);  
    ...
    ASSERT(bits4[0] == 0x123456789ABCDEF0UL);  
    ...
    ASSERT(bits5[0] == 0x123456789ABCDEF0UL);  
...
void test_single_uint64_constructor(TestObjs *objs) {
    ...
    ASSERT(bits1[0] == 0x123456789ABCDEF0UL);  
    ASSERT(!positive_bigint.is_negative());  // Should be positive
    ...
    ASSERT(bits2[0] == 0x123456789ABCDEF0UL); 
    ASSERT(negative_bigint.is_negative());  // Should be negative
...
void test_copy_constructor(TestObjs *objs) {
    ...
    BigInt original1(0x123456789ABCDEF0UL, false); 
    BigInt copy1(original1);  
